...
function Contact() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/contacts/search?query=${searchQuery}`
      );
  ...
  const sortContacts = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/contacts/sort?sortBy=${sortQuery}`
      );
          ...
          <div className="bg-base-100 p-4  shadow-md flex flex-row justify-between w-full ">
            ...
            <div className="dropdown ">
              <div
                tabIndex={0}
                role="button"
                className="btn btn-primary bg-purple-900 text-white border-purple-900 hover:bg-purple-700 hover:border-purple-700 m-1"
              >
                Sort
              </div>
              <ul
